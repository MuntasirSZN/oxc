...
impl<'a> ModuleRecordBuilder<'a> {
    ...
    pub fn visit_import_declaration(&mut self, decl: &ImportDeclaration<'a>) {
        let module_request = NameSpan::new(Atom::from(decl.source.value.as_str().unwrap_or("")), decl.source.span);
    ...
    pub fn visit_export_named_declaration(&mut self, decl: &ExportNamedDeclaration<'a>) {
        let module_request =
            decl.source.as_ref().map(|source| NameSpan::new(Atom::from(source.value.as_str().unwrap_or("")), source.span));
