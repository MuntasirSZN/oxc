...
impl<'a> ClassPropertiesSuperConverter<'a, '_> {
    ...
    fn transform_static_member_expression_impl(
        ...
        let property = ctx.ast.expression_string_literal(property.span, property.name.as_str().into(), None);
    ...
    fn transform_assignment_expression_for_super_static_member_expr(
        ...
        let property =
            ctx.ast.expression_string_literal(member.property.span, member.property.name.as_str().into(), None);
        *expr =
